...
def get_pkl_rootpath(dataset):
    root = os.path.join("./dataset/data", f"{dataset}")
    data_path = os.path.join(root, 'dataset.pkl')

    return data_path
    #return './dataset/data/PA100k/dataset.pkl'
...
def get_pedestrian_metrics(gt_label, preds_probs, threshold=0.5):
    ...
    result.label_acc = (true_pos + true_neg) / (true_pos + false_pos + false_neg + false_pos + eps)
